...
export default function InputCheckbox({
  ...
  return (
    <Checkbox
      checked={checked}
      onChange={onChange}
      className={`group flex cursor-pointer items-center w-fit h-6 relative ${className ?? ""}`}
    >
      <span className="size-5 fill-black group-data-checked:text-primary-color text-transparent border border-gray-1100 rounded-md absolute">
        <svg
          width={22}
          height={22}
          viewBox="0 0 22 22"
      ...
      <p className="text-base pl-8 text-black-1000 font-medium font-inter">{label}</p>
    </Checkbox>
  );
}
