    ...
    <header className="w-full flex h-20 md:h-[90px] lg:h-[100px] bg-white border-b border-gray-1100 z-10 relative">
      <nav className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 flex justify-between items-center">
          ...
          >
           {navLinks.map((link) => (
              <li key={link.label}>
                <Link
                  to={link.path}
                  className={`justify-center transition duration-500 text-sm md:text-base font-semibold font-inter ${
                    isActive(link.path) ? 'text-primary-color' : 'text-neutral-800 hover:text-primary-color'
                  }`}
                >
                  {link.label}
                </Link>
        ...
        <div className="hidden lg:flex items-center gap-5">
          ...
          <Link
            to={"/"}
            className="h-11 px-6 py-2.5 bg-linear-to-r hover:from-primary-color hover:via-primary-color hover:to-primary-color from-black-1000 via-neutral-500 to-black-1000 rounded-full flex items-center gap-2.5 transition duration-300 hover:opacity-90"
          >
            {isAuth ? (
              <>
